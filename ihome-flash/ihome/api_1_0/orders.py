...
def save_order():
    ...
    try:
        ...
        return jsonify(errno=RET.DATAERR, errmsg='日期类型错误')
    ...
    try:
        ...
        current_app.logger.error(e)
        return jsonify(errno=RET.DBERR, errmsg='房屋信息获取失败')
    if not house:
    ...
    try:
        ...
        current_app.logger.error(e)
        return jsonify(errno=RET.DBERR, errmsg='检车出错，请稍后再试')
    if count > 0:
...
def get_user_orders():
    ...
    # 用户的身份，用户想要查询作为房客预定别人的订单，还要作为房东查询别人预定自己房屋的订单
    role = request.args.get('role', '')

    # 查询订单数据
    try:
        if 'landlord' == role:  # 以房东身份查询
            # 先查询属于自己的房子
...
def save_order_comment(order_id):
    ...
    # 因为房屋详情中有订单的评价信息，为了让最新的评价信息展示在房屋详情，所以删除redis中关于本订单房屋信息的详情
    try:
        redis_store.delete('house_info_%s' % order.house_id)
    except Exception as e:
        current_app.logger.error(e)

    return jsonify(errnp=RET.OK, errmsg='OK')
