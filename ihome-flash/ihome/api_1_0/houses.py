...
def get_area_info():
    ...
    # 将数据转换为json字符串
    resp_dict = dict(errno=RET.OK, errmsg='OK', data=area_dict_li)  # 字典转换方法
...
def save_house_image():
    ...
    try:
        ...
        current_app.logger.error(e)
        return jsonify(errno=RET.THIRDERR, errmsg='保存图片错误')
...
def get_house_index():
    ...
    try:
        ...
        ret = None
    if ret:
        # 因为redis保存的是json字符串，所以直接拼接进行字符串返回，节省jsonify转换json数据的资源消耗
...
def get_house_detail(house_id):
    ...
    reps = '{"errno":0, "errmsg":"ok", "data":{"user_id":%s, "house":%s}}' % (user_id, json_house), 200, {"Content-Type": "application/json"}
    return reps
...
def get_house_list():
    """获取房屋列表信息（搜索页面）"""
    start_date = request.args.get('sd', '')  # 获取起始时间
    end_date = request.args.get('ed', '')  # 获取结束时间
    area_id = request.args.get('aid', '')  # 获取区域编号
    sork_key = request.args.get('sk', 'new')  # 排序关键字
    ...
    if area_id:
        try:
            ...
            current_app.logger.error(e)
            return jsonify(errno=RET.PARAMERR, errsmg='区域参数有误')
    ...
    # 分页处理
    try:
        page_obj = house_query.paginate(page=page, per_page=constants.HOUSE_LIST_PAGE_CAPACITY, error_out=Facility)
    except Exception as e:
        current_app.logger.error(e)
        return jsonify(errno=RET.DBERR, errmsg='数据库错误')
    ...
    try:
        ...
        pipeline.expire(redis_key, constants.HOUSE_LIST_PAGE_REDIS_CACHE_EXPIRE)
